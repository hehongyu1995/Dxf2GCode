...
class Shape(object):
    ...
    def Write_GCode_for_geo(self, geo, PostPro):
        ...
        # circle
        logger.debug(self.tr("--Invoke Write_GCode_for_geo()"))
        logger.debug(self.tr("--  length:%s x:%s y:%s")%(geo.length,geo.Ps.x,geo.Ps.y))
    ...
    def Write_GCode(self, PostPro):
        logger.debug(self.tr("--Invoke Write_GCode()"))
        """
